...
const MENU = {
  Entradas: [
    {
      id: 1,
      name: "Burrata com tomate cereja",
      emoji: "🍅",
      price: 60,
      desc: "Burrata cremosa com tomates cherry assados, manjericão fresco e azeite extra virgem.",
    },
    {
      id: 2,
      name: "Tábua de fríos",
    ...
    {
      id: 3,
      name: "Batatas fritas com cheddar e bacon",
      emoji: "🍟",
      price: 40,
      desc: "Batatas fritas com queijo cheddar derretido e bacon em pedaços",
    },
  ],
  Principais: [
    {
    ...
    },
    {
      ...
      emoji: "🥩",
      price: 70,
      desc: "400g de bife madurado. Batatas rústicas com alecrim, chimichurri da casa.",
    },
...
const USE_LOCAL_ASSISTANT = true; // true = modo offline / sin API externa
...
function confirmOrder() {
  ...
  closeModal();
  addBotMessage(
    `✅ ¡Pedido confirmado!\n\n${summary}\n\nEm breve um garçon passara na mesa. Obrigado!`,
  );
}
...
function renderNav() {
  ...
  nav.innerHTML = Object.keys(MENU)
    .map(
      (cat) => `
...
function toggleFlip(event, id) {
  ...
  if (event.target.closest(".add-btn") || event.target.closest(".add-btn-back"))
    return;
...
async function sendMessage() {
  ...
  if (USE_LOCAL_ASSISTANT) {
    const reply = getLocalAssistantReply(text);
    removeTyping();
    addBotMessage(reply);
    isTyping = false;
  ...
  try {
    ...
    const reply =
      data.content?.[0]?.text || "Lo siento, hubo un error. Intentá de nuevo.";
    ...
    removeTyping();
    addBotMessage(
      "Ups, hubo un problema de conexión. ¿Podés intentar de nuevo?",
    );
  }
